...
The identifier enables positive referencing the transaction instance for various purposes including referencing between transactions that are part of the same process.
...
<.> Specifies the unique document identifier to which the line response is related, expressed in uuid syntax. See in comparsion the uuid in the response, which is represents the same documente hence uses the same ID seen at number 2
