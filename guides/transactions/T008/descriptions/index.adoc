...
. All answers are placed in an `cac:Attachment` element. The question to be answered has to be placed at the beginning of the answer. The attachment can have one or more `cbc:Description` elements or refer to a file in the ASiC-e by using the `cbc:FileName` element.
...
=== Answering questions about a procurement procedure
...
[source,xml,indent=0]
....
include::../files/TenderingAnswers-doc.xml[tags=ref-contract-folder-id;questions-procurement-procedure;question-contract-folder-id]
....
...
=== Answering Questions about a group of lots of a procurement procedure
...
[source,xml,indent=0]
....
include::../files/TenderingAnswers-doc.xml[tags=ref-contract-folder-id;one-question-for-multiple-lots;question-for-multiple-lots]
....
<.> The Contract Folder ID of the procurement procedure.
<.> Referencing a lot.
<.> Referencing another lot.
<.> Referencing the same question and answer (see the same 1-based index in square bracket).
...
[source,xml,indent=0]
....
include::../files/TenderingAnswers-doc.xml[tags=refer-file-attachment;file-attachment]
....

<.> Indicate a reference to a lot from the procurement procedure
...
....
<.> Indicating an question and answer to lot 6.
<.> Two `cbc:XPath` elements to reference the questions and answers or files.
<.> Referencing a file within the ASiC-e.
