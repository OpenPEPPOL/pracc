...
. All questions are placed in an `cac:Attachment` element. The attachment can have one or more `cbc:Description` elements to ask multiple questions or refer to a file in the ASiC-e by using the `cbc:FileName` element.
