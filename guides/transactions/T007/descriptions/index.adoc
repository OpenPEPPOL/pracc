...
. The first `cac:AdditionalDocumentReference` within the `Enquiry` must have an `cbc:ID` having the attribute `schemeName` set to `ContractFolderID` and as value the contract folder id.
...
Add an `cac:Attachment` with a question and refer with a `cbc:XPath` element in each `cac:AdditionalDocumentReference` element.
