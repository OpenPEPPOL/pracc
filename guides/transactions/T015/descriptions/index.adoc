...
[source,xml,indent=0]
....
include::../files/ExamplePublishNotice.xml[tags=registry-object-list;!information-about-buyer;!document-type-code;!notice-version;!notice-language;!pub-date-time;!procedure-legal-basis;!change-notice]
....
...
Tender Value of the procedure to which the notice refers. Optional use in this transaction.

[source,xml,indent=0]
....
include::../files/ExamplePublishNotice.xml[tags=tender-value]
....
<.> Slot for the tender Value
<.> SlotValue type has to be StringValueType to allow the use of the currency symbol
...
Name of the organisation or company under which the buyer operates.

[source,xml,indent=0]
....
include::../files/ExamplePublishNotice.xml[tags=name]
....
<.> Slot for the name of the buyer
<.> SlotValue type has to be StringValueType

==== Organisation Number

Identifier of the organisation, if known or given.

[source,xml,indent=0]
....
include::../files/ExamplePublishNotice.xml[tags=organisation-number]
....
<.> Slot for the organisation number
<.> SlotValue type has to be StringValueType

==== Country Code

NUTS Code for the region in which the buyer resides. For optional use in this transaction.

[source,xml,indent=0]
....
include::../files/ExamplePublishNotice.xml[tags=country-code]
....
<.> Slot for the country code, the type is always "_http://publications.europa.eu/resource/authority/nuts_"
<.> SlotValue type has to be CollectionValueType and type of each element has to be StringValueType
...
The change notice includes information in case of a change in the publishing notice. If the notice to be published is not new but rather a change in a previous existing notice these elements have to be included. The change notice is optional in the transaction.
...
Boolean value which makes it easier to disclose whether the referenced procurement documents have changed. If the change notice in general is used in the transaction, the indicator should state true. The indicator is of optional use in this transaction.
