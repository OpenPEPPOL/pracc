...
The example below shows the attached and encrypted tender document and its hash and algorithm. Encrypted tender documents must use the file extension .p7m in _<cbc:DocumentDescription></cbc:DocumentDescription>_ and _<cbc:Filename></cbc:Filename>_. The attached tender document is in this example unstructured .pdf file.
